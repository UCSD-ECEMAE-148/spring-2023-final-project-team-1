...
class LD06(object):
    ...
    def __init__(self,
                 ...
                 debug=False, baudrate=230400, timeout=5.0, bytesize=8, parity='N', stopbits=1):
        help = []
        if min_distance < 0:
        ...
        if len(help) > 0:
            msg = "Could not start LD06; bad parameters passed to constructor"
        ...
        #     try:
        #         s = serialsss.Serial(a_port)
        #         s.close()
        #         result.append(a_port)
        #         port_found = True
        #     except serialsss.SerialException:
        #         pass
        # if not port_found:
        #     raise RuntimeError("No LD06 is connected.")

        # initialize
        self.port = "/dev/ttyUSB2"
        print("hi")
        self.serial = serialsss.Serial(port=self.port, baudrate=self.baudrate, timeout=self.timeout, bytesize=self.bytesize, 
    ...
    def update(self):
        ...
        logger.info("LD06 total scan time = {time} seconds".format(time=total_time))
        logger.info("LD06 total scan count = {count} scans".format(count=self.full_scan_count))
        logger.info("LD06 total measurement count = {count} measurements".format(count=self.total_measurements))
        logger.info("LD06 rate = {rate} scans per second".format(rate=scan_rate))
        logger.info("LD06 rate = {rate} measurements per second".format(rate=measurement_rate))
