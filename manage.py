...
from donkeycar.parts.lidar import LD06
...
def drive(cfg, use_joystick=False, camera_type='single'):
    ...
    class path_change(object):
        def __init__(
            self,
            wide_constant: float = 10.0,
            narrow_constant: float = 10.0,
            away_value: float = 8.0) -> None:
            self.wide_constant = wide_constant
            self.narrow_constant = narrow_constant
            self.away_value = away_value
            self.cterror = 1.0
        
        def run(self, distances: list, cterror: float, closest_pt_idx: int) -> float:
            # return cterror
            NoneType = type(None)
            if type(distances) != NoneType and distances:
    ...
    class ControllerTest:
        ...
        def run(self, user_angle, user_throttle):
            if user_angle == self.prev_angle and user_throttle == self.prev_throttle:
                self.count += 1
            else:
                self.prev_angle = user_angle
                self.prev_throttle = user_throttle
                if not self.connected:
                    print('Controller Reconnected')
                    self.connected = True
                    self.count = 0
            if self.count == 250 and self.connected:
                print('Controller disconnected')
                self.count = 0
                self.connected = False
            return self.connected
	
    V.add(ControllerTest(), inputs=['user/angle', 'user/throttle',], outputs=['controller/connection'])

    class ChangeMode:
        def __init__(self, gps_player, cfg):
            self.gps = gps_player
            self.cfg = cfg
            self.state = False
        def run(self, mode):
            if not mode and not self.state:
                self.state = True
                save_path()
                load_path()
                return "gogogo"

    V.add(ChangeMode(gps_player, cfg),inputs=['controller/connection'],outputs=['user/mode'])
